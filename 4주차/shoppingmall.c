...
void inputProducts(int products, char products_all[MAX_CNT][200]);
void showStatus(int products, int* stock, int minus_all, int plus_all, int* max_id, int* max_sales, int* min_id, int* min_sales, int* minus, char products_all[MAX_CNT][200]);
...
int main() {
    ...
    switch (menu) {
            ...
            case 3:
                showStatus(products, stock, minus_all, plus_all, &max_id, &max_sales, &min_id, &min_sales, minus, products_all);
                break;
            case 4:
                inputProducts(products, products_all);
                break;
            case 5:
                printf("프로그램을 종료합니다.\n");
                return 0;
...
void showStatus(int products, int* stock, int minus_all, int plus_all, int* max_id, int* max_sales, int* min_id, int* min_sales, int* minus, char products_all[MAX_CNT][200]) {
    ...
    printf("가장 많이 판매된 상품 : ID %d (%s), 판매량 %d\n", *max_id + 1, products_all[*max_id], *max_sales);
    printf("가장 적게 판매된 상품 : ID %d (%s), 판매량 %d\n", *min_id + 1, products_all[*min_id], *min_sales);
    for(int i = 0; i < products; i++) {
        if(stock[i] <= 2) {
            printf("상품 ID %d : 상품명 : %s 재고부족(%d)\n", i+1, products_all[i], stock[i]);
        }
    }
}
